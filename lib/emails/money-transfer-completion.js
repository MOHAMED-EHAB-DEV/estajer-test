...
export async function sendMoneyTransferCompletionEmail({
  email,
  providerName,
  orderId,
  transferAmount,
  ...
  const emailSubject =
    userLang === "en"
      ? `💰 Money Transfer Completed - Order #${orderId} - ${companyName}`
      : `💰 تم تحويل الأموال - طلب رقم ${orderId} - ${companyName}`;

  const headerTitle =
    userLang === "en" ? "💰 Money Transfer Completed!" : "💰 تم تحويل الأموال!";
  ...
  const bodyIntro =
    userLang === "en"
      ? `<p style="margin: 0 0 18px 0; font-family: sans-serif;">Hello ${
       ...
       <p style="margin: 0 0 18px 0; font-family: sans-serif;">Great news! The money transfer for your rental order #${orderId} has been completed successfully. The funds have been transferred to your account.</p>
       <p style="margin: 0 0 18px 0; font-family: sans-serif;">Thank you for providing excellent rental services through our platform!</p>`
      : `<p style="margin: 0 0 18px 0; font-family: sans-serif;">مرحباً ${
          providerName ? providerName.split(" ")[0] : ""
        }،</p>
       <p style="margin: 0 0 18px 0; font-family: sans-serif;">خبر رائع! تم إكمال تحويل الأموال لطلب الإيجار رقم ${orderId} بنجاح. تم تحويل الأموال إلى حسابك.</p>
       <p style="margin: 0 0 18px 0; font-family: sans-serif;">شكراً لك على تقديم خدمات إيجار ممتازة من خلال منصتنا!</p>`;
      ...
      <tr style="font-family: sans-serif;">
        <td align="right" style="padding: 10px 0; font-family: sans-serif;">
          <p style="margin: 0; font-size: 18px; font-weight: bold; color: #333333; font-family: sans-serif;">${
            userLang === "en" ? "Transfer Amount:" : "مبلغ التحويل:"
          }</p>
        </td>
        <td align="left" style="padding: 10px 0; font-family: sans-serif;">
          <p style="margin: 0; font-size: 18px; font-weight: bold; color: #f48a42; direction: ltr; font-family: sans-serif;">${transferAmount} ${currencySymbol}</p>
        </td>
      </tr>
    </table>`;
  ...
  const actionButtonsHtml = `
    <div class="cta-button-container" style="text-align: center; margin: 30px 0 10px 0;">
      <a href="${defaultDashboardUrl}" target="_blank" class="cta-button" style="background-color: #F48A42; color: #ffffff !important; padding: 14px 28px; text-decoration: none !important; border-radius: 8px; display: inline-block; font-weight: bold; font-size: 18px; font-family: sans-serif; margin: 0 10px 10px 0;">
        ${userLang === "en" ? "View Dashboard" : "عرض لوحة التحكم"}
      </a>
    </div>`;
  ...
  await transporter.sendMail({
    ...
    <body dir="${userLang === "en" ? "ltr" : "rtl"}">
        <div class="hidden-preheader">
            ${
              userLang === "en"
                ? `Money transfer completed for order #${orderId} on ${companyName}.`
                : `تم تحويل الأموال للطلب رقم ${orderId} على ${companyName}.`
            }
        </div>
                            ...
                            <td class="email-body" style="background-color: #ffffff; padding: 35px 30px; text-align: start; font-family: sans-serif;">
                                ...
                                <p style="margin: 20px 0 0; font-family: sans-serif;">${
                                  userLang === "en"
                                    ...
                                    : "شكراً لك لكونك شريكاً قيماً في منصتنا!"
                                }</p>
                            </td>
                        </tr>
                        ...
                        <tr>
                            <td class="email-footer" style="background-color: #f0f0f0; padding: 30px; text-align: center; font-size: 14px; color: #777777; border-bottom-left-radius: 12px; border-bottom-right-radius: 12px; font-family: sans-serif;">
                                ...
                                <p style="display: block; margin-top:12px; font-family: sans-serif;">
                                    ${
                                      userLang === "en"
                                        ? `You are receiving this message as part of your money transfer notifications. To manage notification preferences, please visit your account settings or <a href="${unsubscribeUrl}" target="_blank" style="color: #F48A42; text-decoration: none; font-weight: bold; font-family: sans-serif;">unsubscribe</a> from some messages.`
                                        : `تستقبل هذه الرسالة كجزء من إشعارات تحويل الأموال. لإدارة تفضيلات الإشعارات، يرجى زيارة إعدادات حسابك أو <a href="${unsubscribeUrl}" target="_blank" style="color: #F48A42; text-decoration: none; font-weight: bold; font-family: sans-serif;">إلغاء الاشتراك</a> من بعض الرسائل.`
                                    }
                                </p>
                            </td>
                        </tr>
