...
export async function sendProductNotificationEmail(
  ...
  const isApproved = status === "approved" || status === "edit-approved";
  const isEdit = status === "edit-approved" || status === "edit-rejected";
  ...
  if (userLang === "en") {
    ...
    emailSubject = isEdit
      ? `Update about your product changes "${productName}" - ${subjectStatus}`
      : `Update about your product "${productName}" - ${subjectStatus}`;
  } else {
    const subjectStatus = isApproved ? "قبول" : "رفض";
    emailSubject = isEdit
      ? `تحديث بخصوص تعديلات منتجك "${productName}" - تم ${subjectStatus}`
      : `تحديث بخصوص منتجك "${productName}" - تم ${subjectStatus}`;
  }
  ...
  const headerTitle =
    userLang === "en"
      ? isApproved
        ...
        : isEdit
          ? "Update About Product Changes"
          : "Update About Your Product"
      : isApproved
        ? isEdit
          ? "🎉 تم قبول تعديلات منتجك!"
          : "🎉 تم قبول منتجك!"
        : isEdit
          ? "تحديث بخصوص تعديلات منتجك"
          : "تحديث بخصوص منتجك";
  ...
  if (userLang === "en") {
    ...
    if (isApproved) {
      if (isEdit) {
        bodyMessage += `<p style="margin: 0 0 18px 0; font-family: sans-serif;">Great news! We're pleased to inform you that your changes for product <strong>"${productName}"</strong> have been approved and are now live on the ${companyName} platform.</p>`;
      } else {
      ...
      if (isEdit) {
        bodyMessage += `<p style="margin: 0 0 18px 0; font-family: sans-serif;">We would like to inform you that after reviewing your changes for product <strong>"${productName}"</strong>, it was decided not to approve them at this time.</p>`;
      } else {
      ...
      if (isEdit) {
        bodyMessage += `<p style="margin: 0 0 18px 0; font-family: sans-serif;">Please review the rejection reason, make the necessary adjustments to your product, and resubmit them.</p>`;
      } else {
    ...
    if (isApproved) {
      if (isEdit) {
        bodyMessage += `<p style="margin: 0 0 18px 0; font-family: sans-serif;">خبر رائع! يسعدنا إعلامك بأنه تم قبول تعديلات منتجك <strong>"${productName}"</strong> وهي الآن مرئية للجميع على منصة ${companyName}.</p>`;
      } else {
      ...
      if (isEdit) {
        bodyMessage += `<p style="margin: 0 0 18px 0; font-family: sans-serif;">نود إعلامك بأنه بعد مراجعة التعديلات المقترحة لمنتجك <strong>"${productName}"</strong>، تقرر عدم قبولها في الوقت الحالي.</p>`;
      } else {
      ...
      if (isEdit) {
        bodyMessage += `<p style="margin: 0 0 18px 0; font-family: sans-serif;">يرجى مراجعة سبب الرفض وإجراء التعديلات اللازمة على منتجك وإعادة تقديمه مرة أخرى.</p>`;
      } else {
