...
export async function sendRequestNotificationEmail({
  ...
  const dashboardUrl = "https://estajer.com/dashboard/requests";
  const requestGuidelinesUrl =
    process.env.REQUEST_GUIDELINES_URL || "https://estajer.com/privacy";
  ...
  const emailSubject =
    userLang === "en"
      ? `Update on your request "${requestName}" - ${subjectStatus}`
      : `تحديث بخصوص طلبك "${requestName}" - تم ${subjectStatus}`;
  ...
  // Language-specific greeting and body content
  const greeting = userLang === "en" ? "Hello," : "مرحبًا،";
  console.log("greeting: ", greeting);
  let bodyMessage = `<p style="margin: 0 0 18px 0; font-family: sans-serif;">${greeting}</p>`;

  if (isApproved) {
    if (userLang === "en") {
      bodyMessage += `<p style="margin: 0 0 18px 0; font-family: sans-serif;">Great news! We're pleased to inform you that your request <strong>"${requestName}"</strong> has been approved and is now ready to be displayed on the ${companyName} platform.</p>
                      <p style="margin: 0 0 18px 0; font-family: sans-serif;">You can now track your request and view its details through your dashboard.</p>`;
    } else {
      bodyMessage += `<p style="margin: 0 0 18px 0; font-family: sans-serif;">خبر رائع! يسعدنا إعلامك بأنه تم قبول طلبك <strong>"${requestName}"</strong> وهو الآن جاهز للعرض على منصة ${companyName}.</p>
                      <p style="margin: 0 0 18px 0; font-family: sans-serif;">يمكنك الآن متابعة طلبك والاطلاع على تفاصيله من خلال لوحة التحكم الخاصة بك.</p>`;
    }
  } else {
    if (userLang === "en") {
      bodyMessage += `<p style="margin: 0 0 18px 0; font-family: sans-serif;">We would like to inform you that after reviewing your request <strong>"${requestName}"</strong>, it was decided not to approve it at this time.</p>`;
      if (reason) {
      ...
      bodyMessage += `<p style="margin: 0 0 18px 0; font-family: sans-serif;">We encourage you to review the <a href="${requestGuidelinesUrl}" target="_blank" style="color: #F48A42; text-decoration: underline; font-family: sans-serif;">request posting guidelines</a> and make the necessary adjustments and resubmit if possible.</p>
      ...
      bodyMessage += `<p style="margin: 0 0 18px 0; font-family: sans-serif;">نود إعلامك بأنه بعد مراجعة طلبك <strong>"${requestName}"</strong>، تقرر عدم قبوله في الوقت الحالي.</p>`;
      if (reason) {
        bodyMessage += `<div style="margin: 0 0 18px 0; padding: 12px 18px; background-color: #fff5f5; border: 1px solid #e0b4b4; border-radius: 8px; color: #721c24; font-family: sans-serif;">
      ...
      bodyMessage += `<p style="margin: 0 0 18px 0; font-family: sans-serif;">نشجعك على مراجعة <a href="${requestGuidelinesUrl}" target="_blank" style="color: #F48A42; text-decoration: underline; font-family: sans-serif;">إرشادات نشر الطلبات</a> وإجراء التعديلات اللازمة وإعادة تقديمه إذا أمكن.</p>
  ...
  await transporter.sendMail({
    ...
    <body dir="${userLang === "en" ? "ltr" : "rtl"}">
        <div class="hidden-preheader">
            ${
              userLang === "en"
                ? `Important update regarding your request "${requestName}" on ${companyName} platform.`
                : `تحديث مهم بخصوص طلبك "${requestName}" على منصة ${companyName}.`
            }
        </div>
