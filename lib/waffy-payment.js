...
class WaffyPayment {
  ...
  async createPayment(paymentData) {
    try {
      ...
      const paymentUrlResult = await this.generatePaymentUrl(
        ...
        paymentData.customRedirectPath,
        paymentData.customRedirectOrigin,
      );
  ...
  async generatePaymentUrl(
    ...
    customerLang,
    customRedirectPath,
    customRedirectOrigin,
  ) {
    try {
      const userToken = await waffyAuth.getUserToken();
      const path = `${customRedirectPath || ""}/payment-completed/${milestoneId}`;
      const base = customRedirectOrigin || this.redirectUrl;
      const url = `${
        this.baseUrl
      }/api/external/contracts/startPayment/${milestoneId}/${
        process.env.WAFFY_CLIENT_ID
      }?redirectUrl=${encodeURIComponent(
        `${base}${customerLang === "en" ? "/en" : ""}${path}`,
