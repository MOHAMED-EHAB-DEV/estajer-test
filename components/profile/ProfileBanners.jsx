...
const fetchBanners = async (userId) => {
  try {
    const res = await fetch(
      `${process.env.NEXT_PUBLIC_APP_URL}/api/home-banners?place=profile&userId=${userId}`,
...
export default async function ProfileBanners({ lang, userId }) {
  const banners = await fetchBanners(userId);
