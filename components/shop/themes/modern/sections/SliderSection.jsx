    ...
    >
      <div className="max-w-screen-2xl mx-auto px-6 md:px-10 lg:px-16 flex flex-col gap-8 md:gap-10">
          ...
          {displayMode === "slider" && displayProducts.length > 1 && (
            <div className="flex items-center gap-2">
              <button
                ...
                className="w-10 h-10 rounded-full border border-neutral-200 bg-white flex items-center justify-center text-neutral-500 hover:text-neutral-900 hover:border-neutral-400 transition-all disabled:opacity-30 disabled:cursor-not-allowed shadow-sm"
              >
                {isAr ? (
        ...
        {displayProducts.length === 0 ? (
          <div className="h-40 border border-dashed border-neutral-200 rounded-3xl flex items-center justify-center text-neutral-400 text-sm bg-[#F9FAFB]">
            {t("addProducts")}
          </div>
        ) : displayMode === "slider" ? (
          <>
            <div className="overflow-hidden" ref={emblaRef}>
              <div className="flex gap-5">
                {displayProducts.map((product, idx) => (
                  <div
                    key={product._id}
                    className="flex-[0_0_220px] md:flex-[0_0_240px] lg:flex-[0_0_calc(25%-1.25rem)] min-w-0 flex flex-col"
                  >
                    <ProductCard
                      theme="modern"
                      product={product}
                ...
                {(scrollSnaps.length > 0 ? scrollSnaps : displayProducts).map(
                  (_, i) => (
                    <button
                      ...
                      style={{
                        width: i === selectedIndex ? "30px" : "6px",
                        backgroundColor:
