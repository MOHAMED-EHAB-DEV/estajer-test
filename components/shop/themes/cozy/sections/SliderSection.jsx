      ...
      return (
        <section className="max-w-screen-2xl w-full mx-auto px-6 md:px-10 lg:px-16 my-6 md:my-12 text-center">
          <div className="flex flex-col items-center gap-2 mb-6">
            <h2 className="text-xl md:text-3xl font-bold text-neutral-800 leading-tight">
              {title}
            </h2>
            <div className="w-12 h-px bg-neutral-300 mx-auto" />
          </div>
          <div className="h-64 bg-[#FAF6F0] rounded-3xl animate-pulse" />
        </section>
      );
    }
    if (previewMode) {
  ...
  return (
    <section className="max-w-screen-2xl w-full mx-auto px-6 md:px-10 lg:px-16 my-6 md:my-12 flex flex-col gap-6 md:gap-10 relative">
      ...
      <div className="flex flex-col items-center text-center">
        {title && (
          <h2 className="text-2xl md:text-4xl font-extrabold text-neutral-900 leading-tight animate-fade-in">
            {title}
          </h2>
        )}
      </div>

      {/* Cozy Nav buttons absolute start overlay */}
      {displayMode === "slider" && displayProducts.length > 1 && (
        <div className="flex items-center gap-2 absolute start-6 top-4 z-20">
          <button
            onClick={() => emblaApi?.scrollPrev()}
            disabled={!canScrollPrev}
            className={`w-9 h-9 border rounded-full flex items-center justify-center transition-colors duration-300 bg-white/95 ${
              canScrollPrev
            ...
            <div className="overflow-hidden" ref={emblaRef}>
              <div className="flex gap-6 py-4">
                {displayProducts.map((product, idx) => (
                  <div
                    key={product._id}
                    className="flex-[0_0_240px] md:flex-[0_0_280px] lg:flex-[0_0_calc(25%-1.2rem)] min-w-0 flex flex-col animate-fade-in"
                  >
                    <ProductCard
                      theme="cozy"
                      product={product}
