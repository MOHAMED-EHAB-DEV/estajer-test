...
const VisitorTicketDetail = ({ lang, translate, id, vid }) => {
  const trans = useTranslations(translate);
  const t = (key) => trans(`support.ticket.${key}`);
  ...
  useEffect(() => {
    const getTicketData = async (id) => {
      try {
        const visitorId = vid || localStorage.getItem("visitorId");
        if (vid) localStorage.setItem("visitorId", vid);

        const res = await fetch(`/api/tickets/${id}?vid=${visitorId}`);
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      const res = await fetch(`/api/tickets/${ticket._id}?vid=${visitorId}`, {
        method: "POST",
      ...
      if (data.success) {
        ...
        const getRes = await fetch(
          `/api/tickets/${ticket._id}?vid=${visitorId}`,
        );
  ...
  return (
    <div
      className="flex flex-col w-full bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden mt-4"
...
export default VisitorTicketDetail;
