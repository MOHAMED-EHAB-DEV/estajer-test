...
const WaffyStatusModal = ({ order, onClose, onSuccess }) => {
  ...
  useEffect(() => {
    if (data) {
      if (data.adminAmount !== undefined && data.adminAmount !== null) {
        setPlatformAmount(data.adminAmount.toString());
        setCustomerAmount((data.customerAmount || 0).toString());
  ...
  const handleSaveCustomAmounts = () => {
    if (platformAmount === "" && customerAmount === "") {
      return handleAction("SAVE_CUSTOM_AMOUNTS", { customAmounts: null });
    }
    ...
    if (settleMode === "fixed") {
      ...
      if (p < 0 || c < 0)
        return toast.error(ToastMessage("لا يمكن إدخال أرقام سالبة"));
      return handleAction("SAVE_CUSTOM_AMOUNTS", {
        customAmounts: { platform: p, owner: o, customer: c },
      });
    } else {
      const pp = parsedPlatform;
        ...
        <div className="flex-1 overflow-y-auto md:p-6 p-4">
          {loading ? (
            ...
            <div className="flex flex-col md:gap-6 gap-4 font-NotoSansArabic text-start">
              <div
                className={`grid ${data.isCompanyWithTax ? "grid-cols-3" : "grid-cols-2"} gap-4 text-start`}
              >
                <div
                ...
                {data.isCompanyWithTax && (
                  <div
                    className={`md:p-4 p-3 rounded-xl border ${data.hasValidBillingAddress ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"}`}
                  >
                    <div className="text-[10px] md:text-xs text-gray-500 mb-1">
                      العنوان الوطني
                    </div>
                    <div className="flex items-center gap-2 font-bold text-xs md:text-sm">
                      {data.hasValidBillingAddress ? (
                        <>
                          <Check className="text-success w-4 h-4" />
                          <span className="text-green-700">متوفر</span>
                        </>
              ...
              <div className="bg-[#F9FAFC] md:p-5 p-4 rounded-xl border border-surfaceBlue">
              ...
              <div className="flex flex-col gap-3 pt-2">
                ...
                {data.waffyStatus !== "COMPLETED" &&
                  data.waffyStatus !== "CASH_OUT_APPROVED" && (
                    <div className="flex flex-col gap-3">
                            ...
                            <div className="flex flex-col gap-1">
                              ...
                              <div className="relative">
                                <input
                                  ...
                                  min="0"
                                  max={
                                    settleMode === "percentage" ? 100 : total
                                  }
                                  step="0.01"
                      ...
                      <button
                        ...
                        className={`text-xs font-semibold text-gray-500 hover:text-darkNavy flex items-center justify-center gap-1.5 transition-colors mx-auto ${
                      ...
                      data.waffyStatus === "REFUND_IN_PROGRESS" ? (
                        <button
                          className="w-full bg-primary text-white md:py-3.5 py-2.5 rounded-xl md:font-bold font-semibold text-sm md:text-base flex items-center justify-center gap-2 shadow-lg hover:opacity-90 transition-all disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-[1.02]"
                        ...
                        <button
                          className="w-full bg-primary text-white md:py-3.5 py-2.5 rounded-xl md:font-bold font-semibold text-sm md:text-base flex items-center justify-center gap-2 shadow-lg hover:opacity-90 transition-all disabled:opacity-50 disabled:cursor-not-allowed transform hover:scale-[1.02]"
                          onClick={handleSaveCustomAmounts}
                        ...
                        >
                          {actionLoading ? (
                            ...
                            <>
                              <Check className="md:w-5 md:h-5 w-4 h-4" />
                              <span>حفظ توزيع المبالغ المخصص</span>
                            </>
                          )}
                        </button>
                      )}
                ...
                {(!data.hasValidLocation || !data.hasValidIban) && (
                  <button
                    className="w-full bg-surfaceBlue text-darkNavy md:py-3.5 py-2.5 rounded-xl md:font-bold font-semibold text-sm md:text-base flex items-center justify-center gap-2 border border-gray-200 hover:bg-gray-100 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
