...
export default function UserNotesModal({ user, onClose }) {
  ...
  useEffect(() => {
    const fetchNotes = async () => {
      try {
        const res = await fetch(`/api/users/${user._id}/notes`);
  ...
  return (
    ...
    >
      ...
      {/* Modal */}
      <div
        className="relative bg-white rounded-2xl shadow-2xl w-full max-w-lg flex flex-col overflow-hidden animate-in fade-in zoom-in duration-200"
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center h-40 text-center gap-3">
              ...
              <p className="text-gray-400 text-[11px] md:text-xs">
                أضف أول ملاحظة على هذا المستخدم
              </p>
            </div>
          ) : (
            notes.map((note) => (
                ...
                {editingNoteId === note._id ? (
                  ...
                  <div className="md:p-4 p-3 space-y-3">
                    <Textarea
                      ...
                      classNames={{
                        inputWrapper:
                          "!bg-gray-50 !border !border-gray-200 !rounded-lg p-0 group-data-[focus=true]:!border-[#f48a42]",
                        input: "text-xs md:text-sm text-darkNavy p-3",
                      }}
                    />
